...
class LocalFacilityService {
  ...
  Future<String> addFavorite(Map<String, dynamic> facility) async {
    ...
    final id =
        facility['id'] ?? DateTime.now().millisecondsSinceEpoch.toString();

    await db.insert('facility_favorites', {
      'id': id,
  ...
  Future<List<Map<String, dynamic>>> getFavorites({
    ...
    if (facilityType != null) {
      ...
      return db.query('facility_favorites', orderBy: 'created_at DESC');
    }
  }
  ...
  Future<bool> saveRecentSearches(List<Map<String, dynamic>> searches) async {
    return _dbService.saveListToPrefs('facility_recent_searches', searches);
  }

  /// 최근 검색 기록 로드
  Future<List<Map<String, dynamic>>?> loadRecentSearches() async {
    return _dbService.loadListFromPrefs('facility_recent_searches');
  }

  /// 검색 필터 설정 저장
  Future<bool> saveSearchFilters(Map<String, dynamic> filters) async {
    return _dbService.saveJsonToPrefs('facility_search_filters', filters);
  }

  /// 검색 필터 설정 로드
  Future<Map<String, dynamic>?> loadSearchFilters() async {
    return _dbService.loadJsonFromPrefs('facility_search_filters');
  }

  /// 예약 정보 저장 (임시)
  Future<bool> saveReservationDraft(Map<String, dynamic> reservation) async {
    return _dbService.saveJsonToPrefs(
      'facility_reservation_draft',
      reservation,
    );
  }
  ...
  Future<Map<String, dynamic>?> loadReservationDraft() async {
    return _dbService.loadJsonFromPrefs('facility_reservation_draft');
  }
