...
class LocalPetService {
  ...
  Future<String> addFeedingRecord(Map<String, dynamic> record) async {
    ...
    await db.insert(
      'feeding_records',
      {
        ...
        'feeding_time': record['feedingTime']?.toString() ?? DateTime.now().toIso8601String(),
        'notes': record['notes'],
  ...
  /// 펫의 급식 기록 조회
  Future<List<Map<String, dynamic>>> getFeedingRecords(String petId, {int limit = 50}) async {
    final db = await _dbService.database;
    return await db.query(
      'feeding_records',
      where: 'pet_id = ?',
  ...
  Future<String> addWalkRecord(Map<String, dynamic> record) async {
    ...
    await db.insert(
      'walk_records',
      {
        ...
        'route': record['route'],
        'start_time': record['startTime']?.toString() ?? DateTime.now().toIso8601String(),
        'end_time': record['endTime']?.toString() ?? DateTime.now().toIso8601String(),
        'notes': record['notes'],
        'created_at': DateTime.now().toIso8601String(),
      },
    );
  ...
  /// 펫의 산책 기록 조회
  Future<List<Map<String, dynamic>>> getWalkRecords(String petId, {int limit = 50}) async {
    final db = await _dbService.database;
    return await db.query(
      'walk_records',
      where: 'pet_id = ?',
  ...
  /// 펫의 건강 기록 조회
  Future<List<Map<String, dynamic>>> getHealthRecords(String petId, {String? recordType}) async {
    final db = await _dbService.database;
    if (recordType != null) {
      return await db.query(
        'health_records',
  ...
  Future<String> upsertActivity(Map<String, dynamic> activity) async {
    final db = await _dbService.database;
    final id = activity['id'] ?? DateTime.now().millisecondsSinceEpoch.toString();
    final now = DateTime.now().toIso8601String();
  ...
  /// 펫의 활동 기록 조회
  Future<List<Map<String, dynamic>>> getActivities(String petId, {String? activityType}) async {
    final db = await _dbService.database;
