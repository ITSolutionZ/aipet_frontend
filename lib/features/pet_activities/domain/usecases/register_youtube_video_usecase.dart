...
class RegisterYouTubeVideoUseCase {
  ...
  /// YouTube 비디오를 등록합니다.
  Future<Result<YouTubeVideoEntity>> call({
    ...
    try {
      ...
      if (videoId == null || videoId.isEmpty) {
        return Result.failure('有効なYouTube URLではありません');
      }
      ...
      final result = await _repository.registerYouTubeVideo(video);
      return Result.success('YouTube動画を登録しました', result);
    } catch (error) {
      return Result.failure('YouTube動画の登録に失敗しました: ${error.toString()}');
    }
  }
