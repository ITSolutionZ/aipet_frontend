...
class _GroomingReservationScreenState
  ...
  void initState() {
    super.initState();
    _controller = GroomingReservationController(ref, context);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      _controller.loadGroomingFacilities();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, ref, child) {
        final state = ref.watch(groomingReservationControllerProvider);

        return Scaffold(
          backgroundColor: AppColors.pointOffWhite,
          appBar: const SoftGradientBackAppBar(title: 'トリミング'),
          body: Column(
            children: [
              // 검색바
              Padding(
