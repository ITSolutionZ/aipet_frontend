...
/// 병원 예약 화면의 상태 데이터
class HospitalReservationState {
  ...
  const HospitalReservationState({
  ...
  HospitalReservationState copyWith({
...
/// 병원 예약 화면의 비즈니스 로직을 관리하는 컨트롤러
class HospitalReservationController {
  final WidgetRef ref;
  final BuildContext context;

  HospitalReservationController(this.ref, this.context);

  /// 병원 시설 데이터 로드
  Future<void> loadHospitalFacilities() async {
    try {
      ...
      _updateState(isLoading: false, error: '병원 시설 데이터를 불러오는데 실패했습니다: $error');
      _showErrorMessage('병원 시설 데이터를 불러오는데 실패했습니다');
    }
  }

  /// 검색어 업데이트
  void updateSearchQuery(String query) {
    final currentState = ref.read(hospitalReservationControllerProvider);
