...
class AiChatState {
  ...
  AiChatState copyWith({
    ...
    final updatedStats = messageStats ?? MessagePaginationService.generateStatistics(updatedMessages);
...
class AiChatNotifier extends _$AiChatNotifier {
  ...
  Future<void> initializeChat() async {
    ...
    if (result.isSuccess && result.dataOrNull != null) {
      ...
      if (initResult.isSuccess) {
        ...
        state = AiChatStateManager.setErrorState(
          currentState: state,
  ...
  Future<void> sendMessage(String content) async {
    ...
    if (result.isSuccess && result.dataOrNull != null) {
      ...
      if (assistantMessageResult.isSuccess) {
        ...
        state = AiChatStateManager.setErrorState(
          currentState: state,
          error: assistantMessageResult.errorOrNull ?? 'Assistant message failed',
          clearTyping: true,
        ).dataOrNull ?? state;
      }
    } else {
  ...
  void optimizeMemoryUsage() {
    ...
    if (optimizedResult.isSuccess) {
      ...
      if (kDebugMode) {
        debugPrint('[AiChatController] Manual memory optimization completed: ${optimizedResult.errorOrNull ?? 'Success'}');
      }
    }
  }
