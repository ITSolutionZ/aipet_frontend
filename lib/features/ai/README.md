...
```dart
// 메시지 리스트 관리
class AiChatController extends Notifier<List<AiMessageEntity>> {
  @override
  List<AiMessageEntity> build() => [];
...
// 타이핑 상태
final isTyping = ref.watch(aiTypingStateProvider);
...
```dart
// 특정 펫을 선택하여 맞춤 상담
ref.read(selectedPetProvider.notifier).state = petProfile;

// 카테고리 선택
ref.read(selectedCategoryProvider.notifier).state = AiCategoryEntity.health;
```
...
- [概要 (Overview)](#overview-japanese)
- [アーキテクチャ (Architecture)](#architecture-japanese)
- [主要機能 (Key Features)](#key-features-japanese)
- [ディレクトリ構造 (Directory Structure)](#directory-structure-japanese)
- [AI チャットフロー (AI Chat Flow)](#ai-chat-flow-japanese)
- [UI 構成 (UI Components)](#ui-components-japanese)
- [使用方法 (Usage)](#usage-japanese)
- [設定 (Configuration)](#configuration-japanese)
...
- **ペット専門システムプロンプト** - ペット関連質問のみ応答
- **リアルタイムタイピングインジケーター**と応答処理
- **日本語ベース応答** - ユーザーフレンドリーインターフェース

#### 🔍 コンテンツフィルタリング

- **ペット関連コンテンツ検証** - 非関連質問ブロック
- **安全な応答システム** - 不適切な内容フィルタリング
...
```txt
ai/
├── ai.dart                              # 機能exportファイル
├── README.md                            # この文書
├── data/
│   ├── data.dart                        # dataレイヤーバレル
│   ├── providers/
│   │   ├── ai_providers.dart            # Riverpodプロバイダー
│   │   └── ai_providers.g.dart          # 生成されたコード
│   ├── repositories/
│   │   └── ai_repository_impl.dart      # AIリポジトリ実装
│   └── services/
│       ├── openai_service.dart          # OpenAI APIサービス
...
│   │   ├── ai_chat_summary_entity.dart  # チャット要約エンティティ
│   │   ├── ai_favorite_entity.dart      # お気に入りエンティティ
│   │   └── ai_favorite_qa_entity.dart   # Q&Aお気に入り
│   └── repositories/
│       ├── repositories.dart            # repositoriesバレル
│       └── ai_repository.dart           # AIリポジトリインターフェース
└── presentation/                        # Presentation Layer
...
```txt
[質問入力] → [コンテンツフィルタリング] → [OpenAI API呼び出し] → [応答表示]
    ↓              ↓                      ↓                ↓
  ユーザー入力   ペット関連検証           GPT応答生成      UI更新
    ↓              ↓                      ↓                ↓
