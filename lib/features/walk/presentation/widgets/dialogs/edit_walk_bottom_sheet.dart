...
class EditWalkBottomSheet extends ConsumerWidget {
  ...
  const EditWalkBottomSheet({super.key, required this.walkRecord, required this.controller});
  ...
  static Future<void> show(
    ...
    return showModalBottomSheet<void>(
      ...
      builder: (context) => EditWalkBottomSheet(walkRecord: walkRecord, controller: controller),
    );
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '散歩記録を編集',
                  style: AppFonts.point(fontSize: AppFonts.xxl, fontWeight: FontWeight.bold),
                ),
                ...
                WalkEditForm(
                  ...
                  onSave: (updatedRecord) => _updateWalk(context, ref, updatedRecord),
                ...
                WalkCoManagerSelector(
                  ...
                  onChanged: (managerId) {
                    ref.read(selectedCoManagerProvider.notifier).state = managerId;
                  },
                ),
                const SizedBox(height: AppSpacing.xl),
                Text(
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.check_circle, size: 16, color: AppColors.pointBlue),
                      const SizedBox(width: AppSpacing.sm),
                      Text(
                        '記録日時: ${_formatDate(walkRecord.createdAt)}',
                        style: AppFonts.base(fontSize: AppFonts.sm, color: AppColors.pointBlue),
                      ),
                    ],
                  ),
                ),
  ...
  void _updateWalk(BuildContext context, WidgetRef ref, WalkRecordEntity updatedWalkRecord) async {
    try {
      ...
      if (context.mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('散歩記録が更新されました'), backgroundColor: AppColors.pointGreen),
        );
        // 상태 초기화
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('更新に失敗しました: $e'), backgroundColor: AppColors.pointPink),
        );
      }
    }
  }
