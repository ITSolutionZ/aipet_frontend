...
class ManageSplashSequenceUseCase {
  ...
  /// 1단계: 초기화 → 2단계: 로딩 애니메이션 → 3단계: 회사로고 3초 → 4단계: 앱로고 3초 → 5단계: 완료
  Stream<SplashResult<SplashState>> execute() async* {
    try {
      // 1단계: 초기화 (로딩 애니메이션 준비)
      yield SplashResult.success('스플래시 초기화 중...', SplashState.initializing());

      // 앱 초기화 작업 수행
      await repository.initializeApp();

      // 2단계: 로딩 애니메이션 표시 - 2초간 표시
      yield SplashResult.success('로딩 애니메이션 표시 중...', SplashState.loading());

      // 로딩 애니메이션 2초 대기
      await Future.delayed(const Duration(seconds: 2));

      // 3단계: 회사 로고 표시 - 무조건 3초간 표시
  ...
  Future<SplashResult<String>> determineNextRoute() async {
    ...
    return SplashResult.success('온보딩 화면으로 이동', '/onboarding');
  }
}
