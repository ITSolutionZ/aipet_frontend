...
import '../../data/services/pet_validation_utils.dart';

/// 펫 등록 컨트롤러
/// UI와 Data Layer 사이의 중간 계층으로 비즈니스 로직을 캡슐화
class PetRegistrationController {
  ...
  /// 현재 등록 상태 가져오기
  PetRegistrationData get currentState => 
  ...
  void setPetSizeWeight({String? size, double? weight}) {
    ...
    ref.read(petRegistrationStateProvider.notifier).setPetSizeWeight(
      size: size,
      weight: weight,
    );
  }
  ...
  void setPetGenderInfo({String? gender, bool? isNeutered}) {
    ...
    ref.read(petRegistrationStateProvider.notifier).setPetGenderInfo(
      gender: gender,
  ...
  void setMicrochipNumber(String? number) {
    if (!PetValidationUtils.isValidMicrochipNumber(number)) {
  ...
  List<String> _getMissingRequiredFields() {
    ...
    if (state.petName == null || state.petName!.isEmpty) missing.add('Pet Name');
