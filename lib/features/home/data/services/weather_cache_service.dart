...
class WeatherCacheService {
  ...
  static const Duration userTriggeredCacheDuration = Duration(minutes: 15); // 사용자 트리거시 15분
  ...
  static Future<WeatherData?> getCached({
    ...
    if (!userTriggered) { // 사용자 트리거시 디스크 캐시 무시
  ...
  static Future<void> setCached({
    ...
    unawaited(_saveDiskCache(locationKey, weatherData, now).catchError((error) {
  ...
  static Future<void> clearAllCache() async {
    ...
    try {
      ...
      final weatherCacheKeys = keys.where((key) => 
  ...
  static Future<WeatherData?> _getDiskCached(String locationKey) async {
    try {
      ...
      final cachedTimeString = prefs.getString('$_cacheTimeKeyPrefix$locationKey');
      ...
      final isExpired = DateTime.now().difference(cachedTime) > offlineCacheDuration;
  ...
  static Future<void> _saveDiskCache(
    ...
    try {
      ...
      await prefs.setString('$_cacheTimeKeyPrefix$locationKey', timestamp.toIso8601String());
    } catch (e) {
