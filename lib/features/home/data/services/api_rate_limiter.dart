...
class ApiRateLimiter {
  ...
  static Result<RateLimitDecision> checkRateLimit(
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[$_tag] ✅ Request allowed for $apiKey - Tokens: ${state.tokensRemaining}, Quota: ${decision.quotaStatus}');
      }
  ...
  /// 백오프 상태 확인
  static Result<RateLimitDecision> _checkBackoffStatus(String apiKey, DateTime now) {
  ...
  static Result<RateLimitDecision> _checkTokenBucket(
    ...
    final tokensToAdd = (timeSinceLastRefill.inSeconds * 0.5).floor(); // 더 보수적으로

    if (tokensToAdd > 0) {
      state.tokensRemaining = min(_burstTokens, state.tokensRemaining + tokensToAdd);
      state.lastRefillTime = now;
    }

    // 우선순위에 따른 토큰 소비량 조정
    final tokensRequired = priority == RequestPriority.high ? 1 :
                          priority == RequestPriority.low ? 2 : 1;
    ...
    if (state.tokensRemaining < tokensRequired) {
      ...
      final decision = RateLimitDecision(
        ...
        retryAfter: retryAfter,
        quotaStatus: QuotaStatus.empty(),
        reason: 'Insufficient tokens (need $tokensRequired, have ${state.tokensRemaining})',
      ...
      if (kDebugMode) {
        debugPrint('[$_tag] 🚫 Token bucket limit reached - retry in ${retryAfter.inSeconds}s');
      }
  ...
  static Result<RateLimitDecision> _checkSlidingWindow(
    ...
    final minuteRequests = history.where((time) => time.isAfter(oneMinuteAgo)).length;
    final hourRequests = history.where((time) => time.isAfter(oneHourAgo)).length;
    ...
    final minuteLimit = isUserTriggered ? _requestsPerMinute + 10 : _requestsPerMinute;
  ...
  static void _incrementBackoff(String apiKey, DateTime now) {
    ...
    _backoffMultiplier[apiKey] = min(_maxBackoffMultiplier, currentMultiplier * 2);
    _lastBackoffTime[apiKey] = now;

    if (kDebugMode) {
      debugPrint('[$_tag] 🔄 Backoff increased for $apiKey: ${_backoffMultiplier[apiKey]}x');
    }
  }
  ...
  /// 쿼터 상태 계산
  static QuotaStatus _calculateQuotaStatus(List<DateTime> history, DateTime now) {
    final oneMinuteAgo = now.subtract(const Duration(minutes: 1));
    final oneHourAgo = now.subtract(const Duration(hours: 1));
    final oneDayAgo = now.subtract(const Duration(days: 1));
  ...
  /// 다음 토큰 리필 시간 계산
  static DateTime? _calculateNextRefillTime(ApiLimiterState state, DateTime now) {
    if (state.tokensRemaining >= _burstTokens) return null;

    final secondsUntilRefill = ((_burstTokens - state.tokensRemaining) / 0.5).ceil();
  ...
  static double _calculateAverageRpm(List<DateTime> history, DateTime now) {
    ...
    final oneHourAgo = now.subtract(const Duration(hours: 1));
    final recentRequests = history.where((time) => time.isAfter(oneHourAgo)).length;
  ...
  static int _calculatePeakRpm(List<DateTime> history, DateTime now) {
    ...
    for (int i = 0; i < 60; i++) {
      ...
      final requestsInWindow = history.where((time) =>
        time.isAfter(windowStart) && time.isBefore(windowEnd)
      ).length;
...
class QuotaStatus {
  ...
  double get minuteUsagePercent => minuteLimit > 0 ? minuteUsage / minuteLimit : 0.0;
