...
import '../../../../../features/scheduling/domain/usecases/get_schedules_usecase.dart';
import '../../../../../features/scheduling/domain/usecases/manage_schedules_usecase.dart';
...
class ScheduleController extends _$ScheduleController {
  ...
  Future<void> updateSchedule(ScheduleEntity schedule) async {
    ...
    try {
      ...
      if (result.isSuccess) {
        final updatedSchedule = result.dataOrNull!;
  ...
  Future<void> updateScheduleStatus(String id, ScheduleStatus status) async {
    ...
    try {
      final result = await _updateScheduleStatusUseCase(id, status);
      if (result.isSuccess) {
        final updatedSchedule = result.dataOrNull!;
        final updatedSchedules = state.schedules.map((s) {
          return s.id == updatedSchedule.id ? updatedSchedule : s;
  ...
  Future<void> cancelSchedule(String id, String reason) async {
    ...
    try {
      final result = await _cancelScheduleUseCase(id, reason);
      if (result.isSuccess) {
        final updatedSchedule = result.dataOrNull!;
        final updatedSchedules = state.schedules.map((s) {
          return s.id == updatedSchedule.id ? updatedSchedule : s;
